...
class AttendanceApp:
    ...
    def load_encodings(self):
        ...
        if not os.path.exists(ENCODINGS_PATH):
            messagebox.showerror("Error", "Encodings not found. Register faces first.")
            return None
    ...
    def _setup_styles(self):
        ...
        style.configure("Title.TLabel", background=COLORS["app_bg"], foreground=COLORS["text"], font=("Segoe UI", 20, "bold"))
        style.configure("Accent.TButton", background=COLORS["accent"], foreground="white", padding=(14, 10), font=("Segoe UI", 10, "bold"))
        style.map("Accent.TButton", background=[("active", COLORS["accent_h"])])

    def _build_layout(self):
        """Create UI layout elements"""
        header = ttk.Frame(self.root, style="App.TFrame")
        header.pack(fill="x", padx=20, pady=15)
        ttk.Label(header, text="Smart Attendance System", style="Title.TLabel").pack(side="left")

        container = ttk.Frame(self.root, style="App.TFrame")
        container.pack(fill="both", expand=True, padx=20, pady=10)

        self.video_card = ttk.Frame(container, style="Card.TFrame")
        self.video_card.pack(side="left", fill="both", expand=True, padx=(0, 10))
        ...
        self.btn_toggle = ttk.Button(control_card, text="Mark Attendance", style="Accent.TButton", command=self.toggle_recognition)
        self.btn_toggle.pack(fill="x", padx=15, pady=25)

        tip_text = "Instructions:\n1. Click 'Mark Attendance'.\n2. Show your face.\n3. One entry per person per day."
        ttk.Label(control_card, text=tip_text, background=COLORS["card_bg"], foreground=COLORS["muted"], font=("Segoe UI", 9)).pack(padx=15, pady=10)
    ...
    def start_recognition(self):
        if self.known_data is None: return
    ...
    def mark_attendance_logic(self, name):
        ...
        if already_marked:
            self.stop_recognition()
            messagebox.showwarning("Already Marked", f"Attendance already marked for {name} today!")
            return
        ...
        self.stop_recognition()
        messagebox.showinfo("Success", f"Attendance Marked Successfully!\nName: {name}\nTime: {time_str}")
    ...
    def update_video(self):
        ...
        if ret:
            ...
            for (top, right, bottom, left), face_enc in zip(face_locs, face_encs):
                matches = face_recognition.compare_faces(self.known_data["encodings"], face_enc, 0.5)
                name = "Unknown"
                
                face_distances = face_recognition.face_distance(self.known_data["encodings"], face_enc)
                ...
                cv2.rectangle(frame, (left, top), (right, bottom), color[::-1], 2)
                cv2.putText(frame, name, (left, top-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color[::-1], 2)

            img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            img = ImageOps.contain(img, (self.video_label.winfo_width(), self.video_label.winfo_height()))
